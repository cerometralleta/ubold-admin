...
public class SqlDefineServiceImpl implements SqlDefineService {
    ...
    public Response createByDataViewCode(String code, JSONObject rowValue) {
        ...
        //列名
        StringBuffer expressionPart = new StringBuffer("(");
        //参数绑定
        Map<String, Object> paramMap = new HashMap<>();
        for (ColumnParam columnParam : dataViewFields) {
            if (columnParam.isInsert()) {
                ...
                expressionPart.append(":").append(columnParam.getField()).append(",");
                paramMap.put(columnParam.getField(), rowValue.get(columnParam.getField()));
            }
        }
        //添加主键列
        createsql.append(sqlDefine.getMasterTableId()).append(",");
        //创建主键表达式
        expressionPart.append(":").append(sqlDefine.getMasterTableId()).append(",");
        //自动生成uuid
        paramMap.put(sqlDefine.getMasterTableId(), GUID.nextId());
        //sql处理
        createsql.deleteCharAt(createsql.lastIndexOf(",")).append(") VALUES ");
        expressionPart = expressionPart.deleteCharAt(expressionPart.lastIndexOf(",")).append(")");
        //end sql
        String sql = createsql.append(expressionPart).toString();
    ...
    public Response ztree(ZtreeParamsRequest ztreeParamsRequest) {
        ...
        List<Map<String, Object>> dataList = namedParameterJdbcTemplateFactory.get(
                ...
                (stringBuilder.toString(), paraMap);
        //异步加载判断是否parent
        if (ztreeParamsRequest.isEnable()) {
